...
public class animesearch extends Thread {
    Action action;
    Context context;
    ...
    public void run() {
        ...
        try {
            json = readJsonFromUrl("https://api.jikan.moe/v3/search/anime?q="+q+"&limit=1");
        } catch (IOException | JSONException e) {
            e.printStackTrace();
        }
        if(json==null){
            l("Empty anime search");
            return;
        }
        try {
    ...
    void replyer(JSONObject obj) throws JSONException {
        ...
        "Score: *"+obj.getString("score")+"*  Episodes: *"+obj.getString("episodes")+"*\n"+
    ...
    String q;
    public animesearch(String q, Action action, Context context) {
